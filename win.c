...
void WndScroll(int x1, int y1, int x2, int y2, int dir)
{
    ...
    if(CheckMousePos(CW->x1 + x1 + xmod, CW->y1 + y1 + ymod, CW->x2 + x2 + xmod,
                     CW->y2 + y2 + ymod))
    {
        m = 1;
        MouseOFF();
    ...
    TTBeginOutput();
    TTScolor(CW->wattr);
    TTScroll(CW->x1 + x1 + xmod, CW->y1 + y1 + ymod, CW->x1 + x2 + xmod, CW->y1 + y2 + ymod, 1,
             dir);
    TTEndOutput();
    ...
    while(!done)
    {
        ...
        switch(event.msgtype)
        {
            ...
            case WND_WM_COMMAND:
            {
                ...
                if(p1 < CW->x1 + xmod + x || p1 > CW->x1 + xmod + x + len - 1 ||
                   p2 != CW->y1 + ymod + y)
                {
                    done = TRUE;
                    break;
                    ...
                    case Key_BS:
                        ...
                        if(i > 0)
                        {
                            ...
                            if(i < strlen(buf))
                            {
                                ...
                                WndPrintf(col + strlen(buf), row, Attr | F_ALTERNATE, "%c", fill);
                            }
                            else
                            {
                                i--;
