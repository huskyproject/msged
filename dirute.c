...
int dir_findfirst(char * filename, int attribute, struct _dta * dta)
{
    ...
    if(DosFindFirst2(filename, &hDir, usAttrib, &InfoBuf, (USHORT)(sizeof(InfoBuf) * cSearch),
                     &cSearch, FIL_STANDARD, (ULONG)NULL) != 0)
    {
        DosFindClose(hDir);
        errno = ENOENT;
        return -1;
...
int dir_findfirst(char * filename, int attribute, struct _dta * dta)
{
    ...
    if(DosFindFirst((unsigned char *)filename, &hDir, usAttrib, &InfoBuf, sizeof InfoBuf, &cSearch,
                    FIL_STANDARD) != 0)
    {
        DosFindClose(hDir);
        errno = DIRUTE_NONE;
...
#elif defined (__RSXNT__) || defined (__CYGWIN__) || defined (__MINGW32__) || \
    (defined (_MSC_VER) && (_MSC_VER >= 1200)) || (defined (__WATCOMC__) && defined (__NT__))
...
static int dir_find_common(struct _dta * dta)
{
    ...
    {
        if(!(InfoBuf.dwFileAttributes & (~(dwFileAttributes | DIR_ARCHVD))))
        {
...
#endif /* if defined (WINNT) || defined (__NT__) */
...
static int match(const char * name, const char * pattern, int attribute, int mode, int rdonly)
{
    char * matpattern;
    char * matname;
    char * cp;
...
int dir_findfirst(char * filename, int attribute, struct _dta * dta)
{
    ...
    while(!fin)
    {
        ...
        if(match(de->d_name, lastbit, attribute, sb.st_mode, access(fullname, W_OK)))
        {
            fin = 1;
        }
    }
...
int dir_findnext(struct _dta * dta)
{
    ...
    while(!fin)
    {
        ...
        if(match(de->d_name, lastbit, ff_attribute, sb.st_mode, access(fullname, W_OK)))
        {
            fin = 1;
        }
    }
...
{
    return stricmp(current_cache + (*((const size_t *)a)), current_cache + (*((const size_t *)b)));
}
