...
{
    return !(tms->tm_wday > 6 || tms->tm_wday < 0 || tms->tm_mon > 11 || tms->tm_mon < 0 ||
             tms->tm_mday > 31 || tms->tm_mday < 0 || tms->tm_hour > 23 || tms->tm_hour < 0 ||
             tms->tm_min > 59 || tms->tm_min < 0 || tms->tm_sec > 59 || tms->tm_sec < 0);
}
...
char * atime(time_t now)
{
    ...
    if(!tm || !valid_date(tm))
    {
        sprintf(tmp, "%s %s %02d %04d %02d:%02d:%02d", day[4], month[0], 1, 1970, 0, 0, 0);
    }
    else
    {
        sprintf(tmp,
...
char * qtime(time_t now)
{
    ...
    {
        sprintf(tmp, "%s %02d %02d:%02d", month[tm->tm_mon], tm->tm_mday, tm->tm_hour, tm->tm_min);
    }
