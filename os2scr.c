...
int TTWriteStr(unsigned long * b, int len, int row, int col)
{
    ...
    for(i = 0; i < len; i++)
    {
        ptr16[i] = (unsigned short)((b[i] & 0xFFUL) | ((b[i] >> 8) & 0xFF00UL));
    }
...
int TTStrWr(unsigned char * s, int row, int col, int len)
{
    ...
    ptr16 = xmalloc16(len);
    memmove(ptr16, s, len);
    VioWrtCharStrAtt((char *)ptr16, (short)len, (short)row, (short)col, (PBYTE)&color, 0);
    xfree16(ptr16);
    return 1;
}
...
int collect_events(int delay)
{
    ...
    if(MouAvail)
    {
        ...
        {
            temp1 =
                ((pMouEvent->fs & MOUSE_BN1_DOWN) || (pMouEvent->fs & MOUSE_MOTION_WITH_BN1_DOWN));
            temp2 =
                ((pMouEvent->fs & MOUSE_BN2_DOWN) || (pMouEvent->fs & MOUSE_MOTION_WITH_BN2_DOWN));
            evt = 0;
...
void MouseInit(void)
{
    USHORT MouMask = MOUSE_MOTION | MOUSE_MOTION_WITH_BN1_DOWN | MOUSE_BN1_DOWN |
                     MOUSE_MOTION_WITH_BN2_DOWN | MOUSE_BN2_DOWN | MOUSE_MOTION_WITH_BN3_DOWN |
                     MOUSE_BN3_DOWN;
...
static unsigned long hsec_time(void)
{
    ...
    DosGetDateTime(&pdt);
    return (pdt.year * 3214080000UL) + (pdt.month * 267840000L) + (pdt.day * 8640000L) +
           (pdt.hours * 360000L) + (pdt.minutes * 6000L) + (pdt.seconds * 100L) + pdt.hundredths;
}
