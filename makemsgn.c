...
static int sent_msg(void)
{
    ...
    rc =
        ChoiceBox("", "WARNING: This message has already been sent! Continue?", "Yes", "No", NULL);
...
#define copystr(d, s) {if(s) {d = xstrdup(s);} \
        else {d = NULL;}}
...
static int findArea(char * tag)
{
    ...
    while(i < SW->groupareas)
    {
        ...
        if(areano >= 0 && !stricmp(arealist[group_getareano(i)].tag, tag))
        {
            break;
        }
...
void change(void)
{
    ...
    if(message->to.internet || message->to.bangpath)
    {
        ...
        message->to.domain = compose_internet_address(message->to.domain, message->isto);
        release(oldptr);
    ...
    if((nname = alias_lookup(tmp, name)) == NULL)
    {
        ...
        if(do_lookup)
        {
            ...
            if(tmp->notfound == 1 && ST->nodepath != NULL)
            {
                ...
                if(m_isdigit(xname[0]) || xname[0] == ':' || xname[0] == '/' || xname[0] == '.')
                {
                    *tmp = parsenode(xname);
                ...
                if(tmp->notfound == 1)
                {
                    *tmp = v7lookup(xname);
                    ...
                    if(tmp->notfound == 0)
                    {
                        ...
                        {
                            sprintf(namefound, "%s at %s found, change?", xname,
                                    show_address(tmp));
                ...
                switch(ch)
                {
                    ...
                    case Key_Ent:
                           ...
                           (*(subj + 2) == '\\' || *(subj + 2) == '/'))
                        {
                            ...
                            {
                                ch2 = ChoiceBox("",
                                                "WARNING: File doesn't exist! Attach anyway?",
                                                "Yes",
                                                "No",
                                                NULL);
...
static int externalEditor(msg * m)
{
    ...
    if(fq != NULL)
    {
        ...
        if(statready &&
           stat(tmpfnm,
                &nstat) == 0 && ostat.st_mtime == nstat.st_mtime && ostat.st_size == nstat.st_size)
        {
            /*
