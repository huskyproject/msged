...
int FidoMsgWriteHeader(msg * m, int type)
{
    ...
    m->msgnum         = n;
    msghead.attrib[0] =
        (unsigned char)((m->attrib.killsent << 7) | (m->attrib.orphan << 6) |
                        (m->attrib.forward << 5) | (m->attrib.attach << 4) |
                        (m->attrib.sent << 3) |
                        (m->attrib.rcvd << 2) | (m->attrib.crash << 1) | (m->attrib.priv));
    msghead.attrib[1] =
        (unsigned char)((m->attrib.ureq << 7) | (m->attrib.areq << 6) | (m->attrib.rcpt << 5) |
                        (m->attrib.rreq << 4) | (m->attrib.freq << 3) | (m->attrib.direct << 2) |
                        (m->attrib.hold << 1) | (m->attrib.local));
...
msg * FidoMsgReadHeader(unsigned long n, int type)
{
    ...
    if(msghead.arrived[0] != 0 || msghead.arrived[1] != 0 || msghead.arrived[2] != 0 ||
       msghead.arrived[3] != 0)
    {
    ...
    {
        m->replyto = FidoUidToMsgn((unsigned long)((msghead.reply[1] << 8) | msghead.reply[0]));
        m->replies[0] = FidoUidToMsgn((unsigned long)((msghead.up[1] << 8) | msghead.up[0]));
    }
...
int FidoAreaSetLast(AREA * a)
{
    ...
    if(msgarr && a->lastread > 0 && a->current > 0 && a->lastread <= msgarrsz &&
       a->current <= msgarrsz)
    {
...
static time_t char_to_timet(unsigned char arr[])
{
    ...
    x = (((unsigned long)arr[0]) << 16) | (((unsigned long)arr[1]) << 24) |
        (unsigned long)arr[2] | (((unsigned long)arr[3]) << 8);
