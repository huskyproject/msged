...
int read_qinfo(FILE * f, struct qinfo * qinfo)
{
    ...
    qinfo->low    = get_word(pbuf);
    pbuf         += 2;
    qinfo->high   = get_word(pbuf);
    pbuf         += 2;
    qinfo->active = get_word(pbuf);
    pbuf         += 2;

    for(i = 0; i < 200; i++)
    {
        qinfo->areas[i] = get_word(pbuf);
        pbuf           += 2;
    }
...
int write_qinfo(FILE * f, struct qinfo * qinfo)
{
    ...
    put_word(pbuf, qinfo->low);
    pbuf += 2;
    put_word(pbuf, qinfo->high);
    pbuf += 2;
    put_word(pbuf, qinfo->active);
    pbuf += 2;

    for(i = 0; i < 200; i++)
    {
        put_word(pbuf, qinfo->areas[i]);
        pbuf += 2;
    }
    assert(pbuf - buf == QINFO_SIZE);
...
static int read_qmsg(FILE * f, struct qmsg * qmsg)
{
    ...
    qmsg->number     = get_word(pbuf);
    pbuf            += 2;
    qmsg->replyto    = get_word(pbuf);
    pbuf            += 2;
    qmsg->replyfrom  = get_word(pbuf);
    pbuf            += 2;
    qmsg->times_read = get_word(pbuf);
    pbuf            += 2;
    qmsg->start      = get_word(pbuf);
    pbuf            += 2;
    qmsg->count      = get_word(pbuf);
    pbuf            += 2;
    qmsg->destnet    = get_word(pbuf);
    pbuf            += 2;
    qmsg->destnode   = get_word(pbuf);
    pbuf            += 2;
    qmsg->orignet    = get_word(pbuf);
    pbuf            += 2;
    qmsg->orignode   = get_word(pbuf);
    pbuf            += 2;
    qmsg->destzone   = *pbuf;
    pbuf++;
    qmsg->origzone = *pbuf;
    pbuf++;
    qmsg->cost     = get_word(pbuf);
    pbuf          += 2;
    attr           = get_word(pbuf);
    pbuf          += 2;
    qmsg->deleted  = attr & 1;
    ...
    qmsg->board    = *pbuf;
    pbuf++;
    memcpy(qmsg->posttime, pbuf, 6);
    pbuf += 6;
    memcpy(qmsg->postdate, pbuf, 9);
    pbuf += 9;
    memcpy(qmsg->whoto, pbuf, 36);
    pbuf += 36;
    memcpy(qmsg->whofrom, pbuf, 36);
    pbuf += 36;
    memcpy(qmsg->subject, pbuf, 73);
    pbuf += 73;
...
static int write_qmsg(FILE * f, struct qmsg * qmsg)
{
    ...
    put_word(pbuf, qmsg->number);
    pbuf += 2;
    put_word(pbuf, qmsg->replyto);
    pbuf += 2;
    put_word(pbuf, qmsg->replyfrom);
    pbuf += 2;
    put_word(pbuf, qmsg->times_read);
    pbuf += 2;
    put_word(pbuf, qmsg->start);
    pbuf += 2;
    put_word(pbuf, qmsg->count);
    pbuf += 2;
    put_word(pbuf, qmsg->destnet);
    pbuf += 2;
    put_word(pbuf, qmsg->destnode);
    pbuf += 2;
    put_word(pbuf, qmsg->orignet);
    pbuf += 2;
    put_word(pbuf, qmsg->orignode);
    pbuf += 2;
    *pbuf = qmsg->destzone;
    pbuf++;
    *pbuf = qmsg->origzone;
    pbuf++;
    put_word(pbuf, qmsg->cost);
    pbuf += 2;
    attr  = (qmsg->deleted ? 1 : 0) + (qmsg->outnet ? 2 : 0) + (qmsg->netmail ? 4 : 0) +
            (qmsg->priv ? 8 : 0) + (qmsg->rcvd ? 16 : 0) + (qmsg->echo ? 32 : 0) +
            (qmsg->local ? 64 : 0) +
            (qmsg->xx1 ? 128 : 0) + (qmsg->killsent ? 256 : 0) + (qmsg->sent ? 512 : 0) +
            (qmsg->attach ? 1024 : 0) + (qmsg->crash ? 2048 : 0) + (qmsg->rreq ? 4096 : 0) +
            (qmsg->areq ? 8192 : 0) + (qmsg->rrcpt ? 16384 : 0) + (qmsg->xx2 ? 32768U : 0);
    put_word(pbuf, attr);
    pbuf += 2;
    *pbuf = qmsg->board;
    pbuf++;
    memcpy(pbuf, qmsg->posttime, 6);
    pbuf += 6;
    memcpy(pbuf, qmsg->postdate, 9);
    pbuf += 9;
    memcpy(pbuf, qmsg->whoto, 36);
    pbuf += 36;
    memcpy(pbuf, qmsg->whofrom, 36);
    pbuf += 36;
    memcpy(pbuf, qmsg->subject, 73);
    pbuf += 73;
    assert(pbuf - buf == QMSG_SIZE);
...
static int read_qtext(FILE * f, struct qtext * qtext)
{
    ...
    qtext->length = *pbuf;
    pbuf++;
    memcpy(qtext->text, pbuf, BLOCKLEN);
    pbuf += BLOCKLEN;
...
static int write_qtext(FILE * f, struct qtext * qtext)
{
    ...
    *pbuf = qtext->length;
    pbuf++;
    memcpy(pbuf, qtext->text, BLOCKLEN);
    pbuf += BLOCKLEN;
    assert(pbuf - buf == QTEXT_SIZE);
...
int QuickMsgDelete(unsigned long n)
{
    ...
    if(fseek(hdrfp, (long)(messages[(size_t)(n - 1)] * (long)QIDX_SIZE), SEEK_SET))
    {
        return FALSE;
    }
...
short find_link(unsigned long n)
{
    ...
    if((((long)header.start + (long)header.count) * (long)QTEXT_SIZE) > b.st_size)
    {
        return 0;
    }
...
int QuickMsgWriteHeader(msg * m, int type)
{
    ...
    header.postdate[0] = 8;
    sprintf(header.postdate + 1, "%02d-%02d-%02d", ts->tm_mon + 1, ts->tm_mday,
            (ts->tm_year % 100));
