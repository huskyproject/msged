...
void makefreq(void)
{
    ...
    if((m = readmsg(CurArea.current)) != NULL)
    {
        ...
        redraw  = 1;
        starty  = 0;
        hWnd2   = WndOpen(0, 6, 79, maxn - starty + 7, SBDR, cm[LS_BTXT], cm[LS_NTXT]);
        sprintf(txt, " File Request from %s, %s ", m->isfrom, show_address(&m->from));
        ...
        while(!done)
        {
            if(redraw)
            {
                ...
                hWnd =
                    WndOpen(0, 6, 79, maxn - starty + 7, SBDR | NOSAVE, cm[LS_BTXT], cm[LS_NTXT]);
                sprintf(txt, " File Request from %s, %s ", m->isfrom, show_address(&m->from));
                WndTitle(txt, cm[LS_NTXT]);
        ...
        else         /* end gotdot (found a area to make req. in) */
        {
            ...
            {
                ChoiceBox("",
                          "No file request area defined. " "Use the FreqArea keyword.",
                          "  Ok  ",
                          NULL,
                          NULL);
            }
