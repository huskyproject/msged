...
static void fc_add_area(s_area * fc_area, int netmail, int local)
{
...
#ifdef USE_MSGAPI
       && fc_area->msgbType != MSGTYPE_SQUISH && fc_area->msgbType != MSGTYPE_JAM
#endif
      )
    {
        return;
    ...
    if((fc_area->group != NULL) && (SW->areafilegroups) && (strcmp(fc_area->group, "\060") != 0))
    {
...
void check_fidoconfig(char * option_string)
{
    ...
    printf(
        "\r\aError! This version of "PROG " has been compiled\n"
    ...
    if(fc_config != NULL)
    {
        if(check_type & 1)      /* load settings */
        {
            ...
            if(fc_config->addrCount)
            {
                alias =
                    xrealloc(alias,
                             (SW->aliascount + fc_config->addrCount) * sizeof(FIDO_ADDRESS));

                for(i = 0; i < fc_config->addrCount; i++)
                {
                    fc_copy_address(alias + SW->aliascount + i, fc_config->addr + i);
                }
            ...
            {
                release(ST->echotoss);
                ST->echotoss = pathcvt(xstrdup(fc_config->echotosslog));
            }
            ...
            /* fido user list */
            if(fc_config->nodelistDir != NULL && fc_config->fidoUserList != NULL)
            {
                release(ST->fidolist);
                ST->fidolist =
                    xmalloc(strlen(fc_config->nodelistDir) + strlen(fc_config->fidoUserList) + 1);
...
void check_fidoconfig(char * option_string)
{
    ...
    if(fc_config_nodelistDir != NULL && fc_config_fidoUserList != NULL)
    {
        release(ST->userlist);
        ST->userlist = xmalloc(strlen(fc_config_nodelistDir) + strlen(fc_config_fidoUserList) + 1);
...
static void read_fidoconfig_file(char * filename, int check_type)
{
    ...
    fclose(f);
} /* read_fidoconfig_file */

#endif /* ifdef USE_FIDOCONFIG */
